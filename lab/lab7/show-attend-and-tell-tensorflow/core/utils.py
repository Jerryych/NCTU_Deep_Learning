...
def sample_coco_minibatch(data, batch_size, batch_no):
    #data_size = data['features'].shape[0]
    #mask = np.random.choice(data_size, batch_size)
    #features = data['features'][mask]
    #file_names = data['file_names'][mask]
    data_path = './data/test/batch_%d.features.hkl' % batch_no
    file_names = data['file_names'][batch_no: batch_no + batch_size]
    #return features, file_names
    return hickle.load(data_path), file_names
