    ...
    with tf.Session() as sess:
        ...
        for split in ['train', 'val', 'test']:
            ...
            for start, end in zip(range(0, n_examples, batch_size),
                ...
                no_list = range(start, end)
                len_batch = len(feats)
                if split == 'train':
                    for f in xrange(len_batch):
                        hickle.dump(feats[f], './data/%s/img_%d.features.hkl' % (split, no_list[f]))
                        if f == 0 or f == (len_batch - 1):
                            print '	Saved... ./data/%s/img_%d.features.hkl' % (split, no_list[f])
                else:
                    hickle.dump(feats, './data/%s/batch_%d.features.hkl' % (split, start))
		    print '	Saved... ./data/%s/batch_%d.features.hkl' % (split, start)
