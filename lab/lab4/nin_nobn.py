...
def cifar_eval(sess):
	...
	for i in range(1, 11):
		...
		acc = acc + accuracy.eval(session=sess, feed_dict={x: image, y: label, keep_prob: 1.0, data_aug: not ON})
	return acc / 10
...
for i in xrange(1, 31281):
	batch = next_batch(i)
	_, lo, acc, summary = sess.run([train_step, cross_entropy, accuracy, merged], feed_dict={x: batch[0], y: batch[1], keep_prob: 0.5, data_aug: ON, W_decay: 0.0001, learning_rate: 0.1})
	if i % 391 == 0:
...
for i in xrange(31281, 47312):
        batch = next_batch(i)
        _, lo, acc, summary = sess.run([train_step, cross_entropy, accuracy, merged], feed_dict={x: batch[0], y: batch[1], keep_prob: 0.5, data_aug: ON, W_decay: 0.0001, learning_rate: 0.01})
        if i % 391 == 0:
                train_writer.add_summary(summary, i / 391)
                acct.append(cifar_eval(sess))
		loss.append(lo)
...
for i in xrange(47312, 64125):
        batch = next_batch(i)
        _, lo, acc, summary = sess.run([train_step, cross_entropy, accuracy, merged], feed_dict={x: batch[0], y: batch[1], keep_prob: 0.5, data_aug: ON, W_decay: 0.0001, learning_rate: 0.001})
        if i % 391 == 0:
                train_writer.add_summary(summary, i / 391)
                acct.append(cifar_eval(sess))
		loss.append(lo)
