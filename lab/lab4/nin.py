...
def act_func(arg, A=0):
	if A == 0:
		return tf.nn.relu(arg)
	elif A == 1:
		return tf.nn.elu(arg)
	elif A == 2:
		return tf.maximum(0.01 * arg, arg)
...
if BN:
	bn1 = tf.contrib.layers.batch_norm(inputs=conv1, center=True, scale=True, epsilon=1e-5, updates_collections=None, is_training=bn)
else:
	bn1 = conv1
...
if BN:
	bn11 = tf.contrib.layers.batch_norm(inputs=mlp11, center=True, scale=True, epsilon=1e-5, updates_collections=None, is_training=bn)
else:
	bn11 = mlp11
...
if BN:
	bn12 = tf.contrib.layers.batch_norm(inputs=mlp12, center=True, scale=True, epsilon=1e-5, updates_collections=None, is_training=bn)
else:
	bn12 = mlp12
...
if BN:
        bn2 = tf.contrib.layers.batch_norm(inputs=conv2, center=True, scale=True, epsilon=1e-5, updates_collections=None, is_training=bn)
else:
        bn2 = conv2
...
if BN:
        bn21 = tf.contrib.layers.batch_norm(inputs=mlp21, center=True, scale=True, epsilon=1e-5, updates_collections=None, is_training=bn)
else:
        bn21 = mlp21
...
if BN:
        bn22 = tf.contrib.layers.batch_norm(inputs=mlp22, center=True, scale=True, epsilon=1e-5, updates_collections=None, is_training=bn)
else:
        bn22 = mlp22
...
if BN:
        bn3 = tf.contrib.layers.batch_norm(inputs=conv3, center=True, scale=True, epsilon=1e-5, updates_collections=None, is_training=bn)
else:
        bn3 = conv3
...
if BN:
        bn31 = tf.contrib.layers.batch_norm(inputs=mlp31, center=True, scale=True, epsilon=1e-5, updates_collections=None, is_training=bn)
else:
        bn31 = mlp31
...
if BN:
        bn32 = tf.contrib.layers.batch_norm(inputs=mlp32, center=True, scale=True, epsilon=1e-5, updates_collections=None, is_training=bn)
else:
        bn32 = mlp32
