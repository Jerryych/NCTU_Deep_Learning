    ...
    for i in range(MAX_EPISODES):
        ...
        for j in itertools.count():
            ...
            if replay_buffer.size() > MINIBATCH_SIZE:
                ...
                scaled_out_batch = behavior_actor.predict(s_batch)
                # 2. Use behavior critic to compute action gradients
                action_gradients = behavior_critic.action_gradients(s_batch, scaled_out_batch)
                # 3. Update behavior actor
                behavior_actor.train(s_batch, action_gradients[0])
                # ------------------------
            ...
            if done:
                print '| Reward: %5.2i' % int(ep_reward), ' | Max Reward: %6.2i' % int(maxreward), ' | Episode %5d' % (i + 1), ' | Qmax: %5.4f' % (ep_avg_max_q / float(j))
                break
